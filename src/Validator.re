...
let make = (~budgetData, ~transactionData, ~render, _children) =>
  ReasonReact.{
    ...component,
    initialState: () => currentMonthYear(),
    reducer: (action, _) => ReasonReact.Update(action),
    render: ({state: (year, month), reduce}) => {
      let (budgets, warnings) = Budget.parseBudgets(budgetData);
      let (categoryMap, transactionWarnings, months) =
        Transactions.parseTransactions(year, month, transactionData);
      <div>
        (
          warnings !== [] && transactionWarnings !== [] ?
            <div className=Glamor.(css([padding("8px 16px"), backgroundColor("#eee")]))>
              (str("Warnings:"))
        ...
        <div className=Glamor.(css([flexDirection("row"), justifyContent("center")]))>
          <button onClick=(reduce((_) => month === 0 ? (year - 1, 11) : (year, month - 1)))>
            (str("<"))
          </button>
          (str @@ string_of_int(year) ++ (":" ++ string_of_int(month + 1)))
          <button onClick=(reduce((_) => month === 11 ? (year + 1, 0) : (year, month + 1)))>
            (str(">"))
          </button>
        </div>
        (spacer(8))
